...
class YahtzeeTest {
    ...
    @Test
    void calculateScore_withCategoryPair() {
        int[] dices = new int[]{3, 4, 6, 2, 6};
        int score = Yahtzee.calculateScore(dices, "Pair");
        assertEquals(12, score);
    }

    @Test
    void calculateScore_withCategoryTwoPairs() {
        int[] dices = new int[]{1, 1, 2, 2, 3};
        int score = Yahtzee.calculateScore(dices, "TwoPairs");
        assertEquals(6, score);
    }

    @Test
    void calculateScore_noMatchingCategory() {
        int[] dices = new int[]{3, 4, 6, 2, 6};
        Exception e = assertThrows(IllegalArgumentException.class, () -> Yahtzee.calculateScore(dices, "SomeCategory"));
        assertEquals("Unexpected value: SomeCategory", e.getMessage());
    }
}